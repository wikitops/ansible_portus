...
SUPPORTED_OS = {
  "debian"   => {box: "ubuntu/stretch64", bootstrap_os: "debian", user: "vagrant"},
...
$os                     = "ubuntu"
$num_instances          = 1
$instance_name_prefix   = "portus"
$vm_memory              = 2048
$vm_cpus                = 1
$subnet                 = "10.0.6.1" # For 10.0.6.1X
    ...
    config.vm.define vm_name = "%s%02d" % [$instance_name_prefix, i] do |server|
      ...
      if i == $num_instances
        config.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            "portus" => ["#{$instance_name_prefix}0[1:#{$num_instances}]"]
          }
        end
      end
    end
